...
- Changed: made the root package a workspace coordinator for `apps/*` and `packages/*`; moved the
  ...
  `apps/desktop/`; kept `packages/pr-watch/` as a private workspace package; added a root Vitest
  config for repo-wide tests; and updated current layout guidance to point at
  `apps/desktop/src/`.
